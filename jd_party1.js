    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        if (cookie) {
           ...
           //await TotalBean();
            ...
            await party_lottery(0)
            await party_lottery(1)
...
if ($.isNode() && allMessage) {
 ...
 async function party_lottery(type){
 return new Promise((resolve) => {
     

   $.post(apihelp(`party_lottery`,type),async(error, response, data) =>{
       //$.log(data)
      try {
        if (error) {
...
function apihelp(functionId,type) {
