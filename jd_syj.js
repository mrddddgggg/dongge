...
let zjdhelp= $.isNode() ? (process.env.zjdhelp ? process.env.zjdhelp : "") : ($.getdata('zjdhelp') ? $.getdata('zjdhelp') : "")
...
function helpFriendTuan(activityIdEncrypted='',assistStartRecordId='',assistedPinEncrypted='') {
  ...
  body =`{"fn":"vvipclub_distributeBean_assist","body":{"activityIdEncrypted":"${iGetInnerText(decodeURIComponent(activityIdEncrypted))}","assistStartRecordId":"${assistStartRecordId}","assistedPinEncrypted":"${iGetInnerText(decodeURIComponent(assistedPinEncrypted))}","channel":"FISSION_BEAN","launchChannel":"undefined"},"appid":"swat_miniprogram","client":"tjj_m","clientVersion":"3.1.3","appId":"dde2b","version":"3.0","code":0}`
    await getH5ST(body)

 $.post(taskTuanUrl(fn, fndata), async (err, resp, data) => {
...
function getUserTuanInfo() {
  ...
  return new Promise(async resolve => {
      ...
      body = {"fn":"distributeBeanActivityInfo","body":{"paramData":{"channel":"FISSION_BEAN"}},"appid":"swat_miniprogram","client":"tjj_m","clientVersion":"3.1.3","appId":"dde2b","version":"3.0","code":0}
    await getH5ST(JSON.stringify(body))
    $.post(taskTuanUrl(fn, fndata), async (err, resp, data) => {
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data['success']) {
              ...
              $.log(data.data.id)
              $.log(data.data.assistStartRecordId)
              $.log(data.data.encPin)
            } else {
              $.tuan = true;//活动火爆
...
return new Promise(async resolve => {
    ...
    body = {"fn":"vvipclub_distributeBean_startAssist","body":{"activityIdEncrypted":"wsaJ5_D1vsc=\n","channel":"FISSION_BEAN"},"appid":"swat_miniprogram","client":"tjj_m","clientVersion":"3.1.3","appId":"dde2b","version":"3.0","code":0}
    await getH5ST(JSON.stringify(body))
    $.post(taskTuanUrl(fn, fndata), async (err, resp, data) => {
      try {
        if (err) {
...
async function getH5ST(a) {
 return new Promise((resolve) => {
    let nm= {
        url: `http://42.192.224.175/getH5ST`,
        body:`${a}`,
        headers: {
...
'Referer': 'http://42.192.224.175/getH5ST',
'User-Agent': 'Mozilla/4.0 (compatible; MSIE 9.0; Windows NT 6.1)',
'Log': 'qjXxMnNiW6~uNJTmbBWxW9igKkWuRuXhcH0mWuhuMU2+ajWPWuWxW9igKkTuRuXhcH0mW+l5',
'Host': '42.192.224.175',
...
function taskTuanUrl(function_id, body) {
  ...
  return {
      url:`https://api.m.jd.com/api?functionId=${function_id}&fromType=wxapp&timestamp=1646612379978`,
...
'Referer': 'https://servicewechat.com/wxa5bf5ee667d91626/184/page-frame.html',
...
async function requestAlgo() {
    ...
    let _0x5b175b = {
        ...
        //'body':`{"version":"3.0","fp":${getRandomIDPro()},"appId":"d8ac0","timestamp":${Date.now()},"platform":"applet","expandParams":""}`,
        'body':`{"version":"3.0","platform":"applet","timestamp":${Date.now()},"appId":"dde2b","fp":${getRandomIDPro()},"expandParams":""}`,
    };
...
let _0x55c9fe = ['' ['concat'](_0x2cecaf['toString']()), '' ['concat']($['fp'][_0x5722('?4e', 'n$S*')]()), '' ['concat']('d8ac0'.toString()), '' [_0x5722('?51', '5nJB')]($[_0x5722('?52', 'w3T]')]), '' [_0x5722('?53', 'RRac')](_0x1c4805), _0x4b101c['oVeqz'], '' [_0x5722('?54', 'TMW@')](_0x4fa26b)][_0x5722('?55', '7Rz$')](';')
